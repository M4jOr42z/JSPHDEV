...
public class FileIO {
	...
	public Automobile buildAutoObject(String filename) throws WrongInputException{
		...
		/* process a file in original format */
		try {
	...
	// if not loaded correctly return null
	public Properties parseProps(String filename) {
	...
	public static void main(String[] args) {
		...
		Properties props = io.parseProps("props/corvette.txt");
		if (props.getProperties("xxx") == null)
			System.out.println("NULL!");
		try {
			ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream("corvette.dat"));
			out.writeObject(props);
			ObjectInputStream in = new ObjectInputStream(new FileInputStream("corvette.dat"));
			Properties p1 = (Properties) in.readObject();
			p1.printInfo();
		} catch (Exception e) {
			e.printStackTrace();
		}
